...
static void conv3x3s1_pack1to4_neon_impl(const Mat& bottom_blob, Mat& top_blob, const Mat& kernel, const Mat& _bias, const Option& opt)
{
...
static void conv3x3s2_pack1to4_neon_impl(const Mat& bottom_blob, Mat& top_blob, const Mat& kernel, const Mat& _bias, const Option& opt)
{
    int w = bottom_blob.w;
    int inch = bottom_blob.c;
    int outw = top_blob.w;
                                       ...
                                       const MatMini* bias)
{
    ...
    Option opt;
    opt.num_threads = 1;
    conv3x3s1_pack1to4_neon_impl(bottom_blob, top_blob, kernel_blob, bias ? bias_blob : Mat(), opt);
    return 0;
}
                                       ...
                                       const MatMini* bias)
{
    ...
    Option opt;
    opt.num_threads = 1;
    conv3x3s2_pack1to4_neon_impl(bottom_blob, top_blob, kernel_blob, bias ? bias_blob : Mat(), opt);
    return 0;
}
