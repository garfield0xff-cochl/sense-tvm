...
struct Mat
{
    ...
    {
        return Mat(w, h, 1, elempack, cstep, data + (size_t)p * cstep);
    }
...
int cochl_ncnn_num_threads = 1;

static void convdw3x3s1_pack4_neon_impl(const Mat& bottom_blob, Mat& top_blob, const Mat& kernel, const Mat& _bias, const Option& opt)
{
#if __aarch64__
...
static void convdw3x3s2_pack4_neon_impl(const Mat& bottom_blob, Mat& top_blob, const Mat& kernel, const Mat& _bias, const Option& opt)
{
    int w = bottom_blob.w;
    ...
    const float* bias = _bias;
    // COCHL_CONVDW3X3S2_FALLBACK_BEGIN
    const bool force_fallback = true;
    const bool odd_width = (w & 1) != 0;
    if (force_fallback || odd_width)
    {
        for (int g = 0; g < group; g++)
        {
            Mat out = top_blob.channel(g);
    ...
    // COCHL_CONVDW3X3S2_FALLBACK_END

#ifdef COCHL_DISABLE_CONVDW3X3S2_ASM
    for (int g = 0; g < group; g++)
    {
        Mat outm = top_blob.channel(g);
        float32x4_t _bias0 = bias ? vld1q_f32((const float*)bias + g * 4) : vdupq_n_f32(0.f);
        const float* k0 = kernel.row(g);
        float* outptr0 = outm;
        const Mat img0 = bottom_blob.channel(g);
