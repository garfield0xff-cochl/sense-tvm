...
def translate(model_path: str) -> Tuple:
    model_path = Path(model_path)
    ext = model_path.suffix.lower()

    if ext == ".onnx":
        return translate_onnx(model_path)
    if ext == ".tflite":
        return translate_tflite(model_path)
    if ext in [".pt", ".pth"]:
        return translate_pytorch(model_path)
    raise ValueError(f"Unsupported model format: {ext}")


def translate_onnx(model_path: Path) -> Tuple:
    import onnx
    import tvm
    ...
    # Filter out constants that TVM doesn't store in metadata["relax.expr.Constant"]
    # TVM inlines these as R.const() or uses them in vm.builtin functions
    ...
    for name in weight_order:
        if name in weights:
            ...
            # Skip reshape constants (shape arrays, typically int64 with small size)
            # These are used in vm.builtin.reshape(), not as operation parameters
            if w.dtype in [np.int32, np.int64] and total_size <= 8:
                removed_count += 1
                continue

            # Skip special constants that TVM inlines
            if 'pad_const' in name or 'const_axes' in name or 'shape' in name.lower():
                removed_count += 1
                continue
    ...
    print(
        f"  Weights: {len(filtered_weight_order)} in TVM Constant order "
        f"({removed_count} scalars removed)"
    )

    return ir_mod, input_info, output_info, weights, filtered_weight_order


def translate_tflite(model_path: Path) -> Tuple:
    raise NotImplementedError("TFLite parsing not yet implemented")


def translate_pytorch(model_path: Path) -> Tuple:
